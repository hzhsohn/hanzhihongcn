...
if(0==strcmp($method,'update'))
{
	...
	if($db->query("update tbNews set ip='$ip',title='$title',titleimg='$ttName',remark='$conent',uptime=now() where autoid=$article_id"))
	{
		//$db->query("select @@identity");
		echo '修改成功... <a href="detail.php?id='.$article_id.'">返回文章</a>';
	}
	else
	{
		echo "------- 操作失败 --------";
...
$db=new PzhMySqlDB();	
$db->open_mysql(cfg_db_host,cfg_db_name,cfg_db_username,cfg_db_passwd);
$db->query('select * from tbNews where autoid='.$article_id,0,0);
if($rs=$db->read())
{
	$title=$rs['title'];
	$content=$rs['remark'];
...
<table width="100%" border="1" cellspacing="0" cellpadding="0" style="background-color:#f5f6fa">
  ...
  <tr>
    ...
    <input type="hidden" name="MAX_FILE_SIZE" value="4000000000"> <input type="hidden" name="titleimg" value="<?=$titleimg?>"></td>
        </tr>
       <tr>
