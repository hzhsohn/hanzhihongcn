...
if(0==strcmp($method,'delete'))
{
	...
	$sql='delete from tbFreeback where autoid='.$autoid;
...
$db->query('select *from tbFreeback order by uptime'.$orderby,$showcount,$page);
?>
...
while($rs=$db->read())
{
	...
	echo '<td><a id="titleID'.$i.'" href="detail_freeback.php?id='.$autoid.'">'.$title.'</a></td>';//标题
	echo '<td>'.$nickname.'</td>';//时间
...
?>
<tr bgcolor="#CCCCCC">
<td colspan="4" bgcolor="#FFFFFF"><div><a href="add_freeback.php" >添加新记录</a></div>
<?php
if($showcount>0)
{
	$db->query('select count(*) from tbFreeback',0,0);
	$rsct=$db->read();
	$n=$rsct[0]/$showcount;
