...
if(0==strcmp($method,'delete'))
{
	...
	$sql='delete from tbQuestion where autoid='.$autoid;
...
$db->query('select *from tbQuestion order by uptime'.$orderby,$showcount,$page);
?>
...
while($rs=$db->read())
{
	...
	echo '<td><a id="titleID'.$i.'" href="detail_question.php?id='.$autoid.'">'.$title.'</a></td>';//标题
...
?>
<tr bgcolor="#CCCCCC">
<td colspan="4" bgcolor="#FFFFFF"><div><a href="add_question.php" >添加新记录</a></div>
<?php
if($showcount>0)
{
	$db->query('select count(*) from tbQuestion',0,0);
	$rsct=$db->read();
	$n=$rsct[0]/$showcount;
